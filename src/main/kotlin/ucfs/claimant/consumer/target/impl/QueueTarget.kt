        ...
        producerProvider().use { producer ->
            coroutineScope {
                records.forEach { (record, result) ->
                    val producerRecord = ProducerRecord("$topic.success", record.key(), result.transformedDbObject.toByteArray())
                    launch {
                        val metadata = producer.send(producerRecord)
                        while (!metadata.isDone) { yield() }
                        val data = metadata.get()
                        logger.info("Successfully sent transformed record","topic" to data.topic(),
                                "offset" to "${data.offset()}", "partition" to "${data.partition()}")
                    }
                }
            }
        }
